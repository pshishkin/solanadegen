...
def get_unprocessed_transactions(cur, batch_size=100):
    query = """
    SELECT signature, timestamp FROM transactions 
    WHERE processed = FALSE AND bucket < 5 
...
def get_transaction_details(tx):
    ...
    result = response.json().get('result', {})
    if not result:
        print(f"Transaction {signature} details not found.")
        print(response.json())
        return
...
def extract_transfers(txs):
    with ThreadPoolExecutor(max_workers=1) as executor:
        results = list(executor.map(get_transaction_details, txs))
        print("Random numbers from transactions:", results)
...
def process_trades():
    ...
    txs_with_transfers = extract_transfers(txs)
    cur.close()
    conn.close()
